...
- Files are real copies in app-managed storage; Remove (×) never touches the original file
...
- Users can save and open named links per tab
...
- Tabbed workspace: default tab + user-created tabs; tab deletion with confirmation
- Tab naming (max 20 characters) with color selection
- Files section per tab: drag-and-drop import, open with default app, reorder, remove from box, delete from computer
- Notes section per tab: multiple note cards, create/edit/collapse/delete
- Sketches section per tab: multiple sketch cards, canvas editor with pen/eraser/brush size/clear/undo/redo, collapse/delete
- Links section per tab: named URLs, add/edit/remove, open in system browser
- Clear-tab action with confirmation
- Settings: light/dark theme, file display preferences (show size, show timestamp), editable global shortcut
